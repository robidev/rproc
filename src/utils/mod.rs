        ...
        2 => {
            match cpu.instruction.addressing_type { 
                ...
                virpc::opcodes::ArgumentSize::Int =>  {
                    operand_hex = format!(" {:02X} {:02X} {:02X} {:02X} ", cpu.read_byte(prev_pc), cpu.read_byte(prev_pc + 0x01), cpu.read_byte(prev_pc + 0x02), cpu.read_byte(prev_pc + 0x03));
                    operand = format!("${:08X}  ", cpu.read_int_le(cpu.prev_pc));
                }
            }
        }
        3 => {
